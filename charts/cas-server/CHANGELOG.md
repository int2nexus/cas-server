...
cas-server 는 기동 시 `sqlx::migrate!` 로 `migrations/` 를 적용합니다. 이미지가 그 집합을
바꾸지 않은 버전이면 `없음` 이고, **마이그레이션 파일이 하나라도 늘어난 버전이면** 아래 세
줄을 함께 적습니다 — 도입하는 쪽이 점검창을 잡고 백업 시점을 정하는 데 필요한 정보입니다.
차트만 바뀐 버전(이미지 `변경 없음`)은 언제나 `없음` 입니다.
...
Location 이 오브젝트 스토리지를 직접 가리킵니다. 그 URL 은 유효 시간 동안 CAS 인가를
거치지 않고, 클라이언트가 스토리지 엔드포인트에 직접 닿아야 하며, Range 도 스토리지가
처리합니다. 기본값 `0`(비활성)은 그대로입니다 — 켤 때 볼 것을 `values.yaml` 주석에
적었습니다.

**주의** — 에러 코드 표(`docs/architecture.md` 6장)에 넷을 넣었습니다. `enforce` 에서 나는
`XAmzContentSHA256Mismatch` · `XAmzContentChecksumMismatch`(400), `aws-chunked` 요청에
`x-amz-decoded-content-length` 가 없을 때의 `MissingContentLength`(411), 그리고 같은
`(issuer, subject)` 의 STS 신원 매핑을 다시 등록할 때의 `Conflict`(409)입니다.
**마지막 것은 매니페스트를 재적용하는 구성에서 정상 동작입니다** — 실패로 다루지 마십시오.
