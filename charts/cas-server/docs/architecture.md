...
GC 는 네 단계로 구성되며 비용이 크게 다릅니다.

| 단계 | 하는 일 | 비용의 성질 |
|------|---------|-------------|
| `multipart` | 만료된 미완료 멀티파트 업로드의 파트 회수 | 대상 테이블이 작아 규모를 타지 않습니다 |
| `orphan` | 회수 후보 큐를 확인해 블롭을 물리 삭제 | 삭제·덮어쓰기 건수를 따릅니다 |
| `purge` | 보존 기간이 지난 삭제 레코드 정리 | 전용 인덱스가 덮어 규모를 타지 않습니다 |
| `sweep` | 블롭 테이블 전체를 훑어 참조가 사라진 것을 찾음 | **테이블 크기를 따릅니다** |

참조가 0 이 되는 시점에 서버가 회수 후보를 기록하고, `orphan` 은 그 큐만 확인합니다.
`sweep` 은 그 큐에 등록되지 못한 것을 찾는 단계이고, `gc.fullSweep` 이 낮은 빈도로
돌립니다.

**`sweep` 을 빼면 회수가 늦어지는 것이 아니라 조용히 쌓입니다.** 같은 블롭을 가리키던
마지막 두 참조가 서로 다른 키에서 동시에 지워지면, 두 트랜잭션이 서로의 삭제를 아직 보지
못해 양쪽 다 참조가 남아 있다고 세고 후보가 등록되지 않습니다. 그것을 찾는 경로가 `sweep`
뿐이라 차트가 그 조합을 렌더에서 거부합니다.

`orphan` 은 뺄 수 없습니다 — 큐를 비우는 경로가 그것뿐이라 차트가 렌더에서 거부합니다.
...
>
> 「이미 있다」는 그 블롭이 **기록된 백엔드**를 기준으로 봅니다(이미지 `0.1.31` 이상).
> 백엔드를 여럿 둔 배포에서 버킷을 핀하지 않아 배정이 요청마다 달라져도 판정이 흔들리지
> 않습니다. 대신 **기록된 백엔드가 내려가 있거나 설정에서 빠져 있으면 그 블롭에 대한
> 쓰기는 `503` 입니다** — 다른 백엔드에 사본을 만들어 성공시키지 않습니다.
>
> `0.1.30` 이하는 요청마다 고른 백엔드를 기준으로 봤고, 그래서 다른 백엔드에만 블롭이
> 있으면 본문을 다시 써서 추적되지 않는 사본이 생겼습니다. 백엔드가 하나인 배포는
> 전후가 같습니다.
>
...
| S3 백엔드 응답 불가 | **블롭 존재 조회가 실패하는 경로는 `503 ServiceUnavailable` 입니다**(이미지 `0.1.31` 이상) — 쓰기(PUT·멀티파트 완료)와 `CopyObject` 의 소스 재확인입니다. 그 밖의 경로에서 오브젝트 스토리지가 돌려준 오류는 여전히 `500 InternalError` 로 나갑니다. `0.1.30` 이하는 전부 `500` 이었습니다. 다른 S3 백엔드는 정상 동작합니다 |
| 신규 업로드 | **그 해시의 `blobs` 행이 이미 있으면 기록된 백엔드**입니다(이미지 `0.1.31` 이상). 행이 없는 새 해시일 때만 아래로 고릅니다 — **NAS**: 정상 백엔드 중 가용 공간 최대인 곳. **S3**: 등록 순서 round-robin. 버킷이 특정 백엔드에 핀돼 있으면 그쪽 |
...
`signed_invalid` 는 `cause` 라벨로 원인이 한 겹 더 갈립니다(`key_expired` ·
`signature_mismatch` · `unknown_key`) — 자세한 것은 `docs/usage.md` 의 메트릭 절입니다.
...
| 400 | `InvalidRequest` | 헤더 서명인데 `x-amz-content-sha256` 이 없거나 값이 비었음 (이미지 `0.1.31` 이상). presigned 는 규격상 그 헤더가 없는 것이 정상이라 해당 없습니다 |
...
| `missing_payload_hash` | 헤더 서명인데 `x-amz-content-sha256` 이 없거나 비어 있음 (이미지 `0.1.31` 이상) | 그 헤더를 싣도록 클라이언트 수정. **응답은 `400` 입니다** |
