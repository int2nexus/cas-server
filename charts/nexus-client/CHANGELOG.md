...
image: `jiwonkim97/nexus-client:0.1.10`

이번 릴리스: 버전 삭제 시 "CAS 오브젝트 함께 삭제" 기본값을 해제(off)로 변경 — 실수로 원본 바이트까지 지우는 것을 막고 필요할 때만 명시적으로 켠다.

**동작 변경** — 버전 삭제 다이얼로그의 "CAS 오브젝트도 함께 삭제" 체크박스가 기본 해제로 바뀐다. 기본 삭제 대상(버전 + 버전 전용 샘플 메타)은 그대로이고, CAS 이미지 바이트는 사용자가 체크해야만 삭제된다. 프록시·라우팅 계약 변경 없음.
**마이그레이션** — 없음
**설정 키** — 없음
**호환성** — 없음 (프론트 UI 기본값만 변경, 차트·nexus-server·CAS 계약과 무관)
