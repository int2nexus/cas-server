...
Token은 발급 후 만료(최대 24시간)까지 무효화할 수 없다. 서버가 요청마다 토큰을 검증할 때 DB를 조회하지 않기 때문이며, 이는 대량 적재 시 커넥션 경합을 피하기 위한 선택이다. 따라서 비밀번호를 변경하거나 계정을 삭제해도 이미 발급된 토큰은 만료 시점까지 유효하다.
