...
- 전면 인증  
조회를 포함한 모든 엔드포인트가 인증을 요구한다. 예외는 회원 가입·로그인·프로브 두 경로·OpenAPI 문서와 Swagger UI 셸뿐이다(문서 경로는 설정으로 끌 수 있다). 데이터 변경 권한은 인증 위에 Dataset 소유권 정책을 더해 결정한다.
- 일관된 오류 모델  
...
**소유자가 지정되지 않은(NULL) Dataset은 인증된 사용자 누구나 변경하고 삭제할 수 있다.** 마지막 Version을 삭제하면 Dataset도 함께 사라지므로, 사실상 누구나 그 카탈로그를 없앨 수 있다는 뜻이다. CAS에 있는 원본 객체는 삭제 요청에 `delete_cas=true`를 **명시한 경우에만** CAS로 삭제 요청이 가고(기본값은 `false`, 보존), 그때도 실제 정리는 CAS 내부 GC가 수행한다.
...
**소유권 이전은 superuser 전용이다.** 소유자는 Dataset 생성 시점에 생성자로 한 번 정해지고, 일반 사용자는 그 값을 바꿀 수 없다. superuser가 설정된 배포에서는 `PUT /api/v1/admin/datasets/{dataset_id}/owner`로 소유자를 지정한다 — 기존 소유자가 있으면 양도, 소유자가 NULL이면 인수이며 **둘은 같은 연산이다**(인수 쪽이 위 문단의 트레이드오프에 해당한다). superuser가 설정되지 않은 배포에서는 이 경로가 누구에게나 403이므로, 소유자를 바꾸려면 superuser를 먼저 설정해야 한다.
