...
| `delete` | 해제 | **삭제** | `editor` 이상 | 세션 자체를 정리. 회수하지 않은 편집도 함께 사라진다 |
...
- 전면 인증  
조회를 포함한 모든 엔드포인트가 인증을 요구한다. 예외는 회원 가입·로그인·프로브 두 경로·OpenAPI 문서와 Swagger UI 셸뿐이다(문서 경로는 설정으로 끌 수 있다). 데이터 변경 권한은 인증 위에 계정 역할을, 파괴적 동작에는 담당자 검사를 더해 결정한다.
- 일관된 오류 모델  
HTTP 상태 코드와 함께 구체적인 오류 정보를 제공한다.

## 10. 권한과 불변성 정책
Nexus는 계정 역할로 무엇을 할 수 있는지를 정하고, 되돌릴 수 없는 동작에만 담당자 검사를 더한다.  
...
**조회를 포함한 모든 엔드포인트가 토큰을 요구한다.** 예외는 회원 가입, 로그인, 프로브 두 경로(`/_internal/live`, `/_internal/health`), OpenAPI 문서(`/api-docs/openapi.json`), 그리고 Swagger UI 정적 셸(`/swagger-ui`, `/swagger-ui/`) 일곱 가지뿐이다. 문서 세 경로는 설정으로 끌 수 있으며, 끄면 등록 자체가 되지 않아 404가 된다. 토큰이 없거나 만료되었으면 401을 반환한다.
...
**삭제는 역할과 담당을 함께 본다.** Version·Sample 삭제는 `editor` 이상이면서 담당자 본인이거나 `admin`이어야 한다. 역할 검사가 먼저인 이유는, 담당자 검사만 두면 `viewer`로 강등된 사람이 자기가 담당인 Dataset을 계속 지울 수 있어 강등의 목적이 무너지기 때문이다. 역할이 동사를 가르고, 담당이 대상을 가른다.

**담당자가 비어 있으면 `admin`만 지울 수 있다.** 담당자가 없다는 것이 더 이상 권한을 열어 주지 않으므로, 그 상태는 위험이 아니라 **인수 대기**라는 정상 상태다. `GET /datasets?unowned=true`로 목록을 만들 수 있다.
...
즉, 역할과 담당자는 누가 변경할 수 있는지를 결정하고, Version 상태는 변경이 가능한지를 결정한다. 두 정책은 서로 독립적으로 동작한다.
...
|쓰기는 계정 역할, 삭제는 역할과 담당자|	소유권 단일 축, 또는 Dataset별 ACL|	한 필드가 출처·담당·인가를 겸하면 협업하려면 담당자를 비워야 하고, 비우는 순간 아무나 쓸 수 있게 된다. 되돌릴 수 없는 동작에만 담당자 검사를 남겨 협업과 안전을 함께 얻는다|
